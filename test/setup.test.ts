...
describe("Setup onboarding", () => {
  ...
  it("resetcalendar returns the campaign to the onboarding gate and whispers the GM an old→new summary (no public broadcast)", () => {
    freshInstall();
    completeSetup();
    const before = chatLog().length;
    resetToDefaults();
    ...
    // Whisper-first: reset is a GM-only whisper now — `!cal send` is the
    // single public broadcast surface. Players are NOT spammed with a reset
    // line, so nothing goes out /direct.
    assert(!newChats.some((c) => /\/direct/.test(c.msg)),
      "resetcalendar must not post any public /direct broadcast");
    const gmWhisper = newChats.find((c) => /\/w gm/.test(c.msg) && /Calendar reset/.test(c.msg));
    assert(gmWhisper, "expected a GM-only reset whisper");
    assert(/Was:/.test(gmWhisper!.msg), "whisper should include the prior date");
    assert(/Now:/.test(gmWhisper!.msg) && /campaign default/.test(gmWhisper!.msg),
      "whisper should include the new campaign-default date");
  });
});
